...
const sceneOptions = ["全部", "见面花束", "订婚花束", "结婚手捧花", "婚车花礼", "表白花束", "男生花束", "生日花束"];
...
export default function CatalogClient() {
  ...
  function uploadPreview(event: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => updateEditing("image", String(reader.result));
  ...
  return (
    <main>
      ...
      <section className="catalog" id="collection">
        ...
        <div className="scene-filter" aria-label="使用场景筛选">{sceneOptions.map(scene => <button onClick={() => setActiveScene(scene)} className={scene === activeScene ? "selected" : ""} key={scene}>{scene}</button>)}</div>
      ...
      {adminOpen && <div className="admin-shell" role="dialog" aria-modal="true" aria-label="店主编辑台"><aside><div className="admin-brand"><span className="brand-mark">F</span><div><strong>FLORA</strong><small>OWNER STUDIO</small></div></div><div className="admin-users"><span>协作店主</span><div><i>LW</i><i>CX</i><b>2 人在线</b></div></div><nav className="admin-nav"><button className="selected">花礼管理 <span>{bouquets.length}</span></button><button>场景管理 <span>{sceneOptions.length-1}</span></button><button>展示设置</button></nav><p className="preview-warning">预览版本<br/><span>编辑在当前页面生效，刷新后复位。</span></p></aside><div className="admin-main"><header><div><small>BOUQUET LIBRARY</small><h2>花礼管理</h2></div><button className="close-admin" onClick={()=>setAdminOpen(false)}>返回顾客页 ×</button></header><div className="admin-workspace"><div className="admin-list"><button className="add-card">+　新增花礼</button>{bouquets.map(item=><button onClick={()=>setEditingId(item.id)} className={item.id===editing.id?"selected":""} key={item.id}><img src={item.image} alt=""/><span><small>{item.id}</small><b>{item.name}</b><em>已上架</em></span></button>)}</div><form className="editor" onSubmit={e=>{e.preventDefault();setSaved(true)}}><div className="editor-head"><div><span className="status-dot"/>正在编辑　{editing.id}</div><button type="submit">{saved?"已保存":"保存更改"}</button></div><div className="editor-grid"><label>花礼名称<input value={editing.name} onChange={e=>updateEditing("name",e.target.value)}/></label><label>色系标签<input value={editing.color} onChange={e=>updateEditing("color",e.target.value)}/></label><label className="full">一句话描述<input value={editing.subtitle} onChange={e=>updateEditing("subtitle",e.target.value)}/></label><label className="full">详细介绍<textarea value={editing.description} onChange={e=>updateEditing("description",e.target.value)}/></label><div className="full upload-field"><span>封面图片</span><div><img src={editing.image} alt="封面预览"/><label className="upload-button">上传新图片<input type="file" accept="image/*" onChange={uploadPreview}/></label><small>JPG / PNG / WEBP，建议使用竖版花束照片</small></div></div><fieldset className="full"><legend>价格信息</legend><label>整体参考价<input value={editing.price} onChange={e=>updateEditing("price",e.target.value)}/></label><label>体量影响<input value={editing.sizePrice} onChange={e=>updateEditing("sizePrice",e.target.value)}/></label><label>花材影响<input value={editing.materialPrice} onChange={e=>updateEditing("materialPrice",e.target.value)}/></label></fieldset><fieldset className="full scene-checks"><legend>所属场景（可多选）</legend>{sceneOptions.slice(1).map(scene=><label key={scene}><input type="checkbox" checked={editing.scenes.includes(scene)} onChange={e=>updateEditing("scenes",e.target.checked?[...editing.scenes,scene]:editing.scenes.filter(s=>s!==scene))}/>{scene}</label>)}</fieldset></div></form></div></div></div>}
    </main>
  );
}
