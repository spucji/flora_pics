...
export default function CatalogClient({ ownerMode = false }: { ownerMode?: boolean }) {
  ...
  async function deleteBouquet() {
    if (!editing || !window.confirm(`确定删除「${editing.name}」吗？删除后顾客页面将不再展示。`)) return;
    const nextBouquets = bouquets.filter(item => item.id !== editing.id);
    setCatalogError("");
    try {
      const response = await fetch("/api/catalog", { method:"PUT", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ scenes, bouquets:nextBouquets }) });
      const payload = await response.json();
      if (!response.ok || !isCatalogState(payload.catalog)) throw new Error(payload.error || "删除失败");
      setScenes(payload.catalog.scenes);
      setBouquets(payload.catalog.bouquets);
      setEditingId(payload.catalog.bouquets[0]?.id || "");
      setAiDraft(null);
      setSaved(true);
    } catch (error) {
  ...
  return (
    <main onClickCapture={event => { const target = event.target as HTMLElement; if (target.closest(".consult-button")) setConsultOpen(true); if (ownerMode && target.closest(".close-admin")) window.location.assign("/owner"); }}>
      ...
      {adminOpen && adminView==="bouquets" && editing && <button type="button" className="delete-bouquet" onClick={deleteBouquet}>删除当前花礼</button>}
    </main>
  );
}
